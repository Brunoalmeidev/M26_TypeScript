namespace casting {
    ...
    (idade as number).toFixed(); // Converte 'idade' p/ 'number e chama o método 'toFixed'
    (idade as string).length; // Converte 'idade' p/ 'string' e acessa a propriedade 'length'

    /* (idade as string[]).forEach(x => {
        console.log(x); // Comentado porque 'idade' ñ é um array de string
    }) */

    let nome: string = 32 as unknown as string;
    nome;
}
